...
L_plot=np.zeros((N+1,3))        # it seems python will run code between def and for
                                # loop if I write L_plot = L_s here
print(L_plot[100,:])
...
for i in range(1,N+1):
    ...
    L_plot[i,:] = L_s[i,:]/np.linalg.norm(L_s[i,:])
...
linetrace=ax.plot(cordvec[:,0,2],cordvec[:,1,2],cordvec[:,2,2],'g.')
lineLtrance=ax.plot(L_plot[:,0],L_plot[:,1],L_plot[:,2],'c.')
...
ax.set_zlim3d([0, 1.5])
##ax.axis('equal')
line_ani = animation.FuncAnimation(fig, update_line, list(range(0,N,23)),
