...
for nni in range(nng):
    ...
    gyrovec2[:,nni] = np.dot(rotation_matrix([0,-1.0*np.sin(np.radians(10)),
                            -1.0*np.cos(np.radians(10))],np.radians(130)),
                            gyrovec[:,nni])
...
markersizearray =  1800*np.pi*xx*xx*hydrogenN3l2(xx)*hydrogenN3l2(xx)
#plt.figure(4)
#plt.plot(markersizearray)
ax3.scatter(*gyrovec2[:,:50],s=markersizearray,color='k',alpha=0.1)
...
plt.show()
#plt.savefig(r'C:\Documents and Settings\user\My Documents\tony\2014\Xelatexfolder\gyrologo2.png')
