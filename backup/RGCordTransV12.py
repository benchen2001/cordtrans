...
def CK(rotvec):
    ...
    Important note, quarternion is with its couterclockwise active sense here
...
class RigidBodyObject:
    ...
    def HasbunEulerEquationODEsolve(self):
        ...
        self.L_Bnorm = np.array([row[:]/np.linalg.norm(row) for row in L_Bt])        
    ...
    def EulerDCMiter(self): 
        ...
        for i in range(1,self.N+1):
            ...
            print(np.dot(self.Tau_s_temp,self.L_s[i-1,:]))
