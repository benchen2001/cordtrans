...
tn=1.0                      # end of simulation time, take longer time if tn > 10 seconds
...
def eulerW2bodyW(eulerW):
    ...
    wbz = w[0,2]# eulerW[0]*np.cos(eulerW[1])
    return wbx,wby,wbz
...
#    print(TrackMat_temp)
    TrackMat_temp = np.dot(TrackMat_temp,CK(w[i,:]*h))        #wi here, _body_hasbun
...
baxes_hasbun = [ax.plot(*[ [0+cordvec[0,j,2]/2,(cordvec[0,j,i]+cordvec[0,j,2])/2] for j in range(3) ]) for i in range(7,10)]
...
line_ani = animation.FuncAnimation(fig2, update_line, list(range(1,N,30)),
