...
tn=1.8                    # end of simulation time, take longer time if tn > 10 seconds
...
#orien = np.array([-.9525,0,0])    #starting orientation vector of top from lab xyz
orien = np.array([-np.radians(15),0,0])    #starting orientation turn of top from space xyz
w = np.zeros([N+1,3])
w[0,2]= 2*np.pi*freq         #wz is the symetry axis

Hasbuncompare = 1            #turn on hasbun's result for comparison, on = 1, off = 0
...
#Hasbun's code
Lx,Ly,Lz,Lzppx,Lzppy,Lzppz = top.HasbunMatlabTop(Iy,Iz,g,M,arm,M*g*arm,tn,h,w[0,2])
Lzpp = np.array([Lzppx,
                 Lzppy,
                 Lzppz])
...
setp(polylines,lw=5)
linetrace=ax.plot(cordvec[:,0,2],cordvec[:,1,2],cordvec[:,2,2],'b.',markersize=2)
#lineLtrance=ax.plot(L_plot[:,0],L_plot[:,1],L_plot[:,2],'k.',markersize=2)
#HasbunLtrance=ax.plot(Lzppnorm[:,0],Lzppnorm[:,1],Lzppnorm[:,2],'k.',markersize=2)
...
#line_ani.save('3Dtopoo.mp4',writer = 'ffmpeg',fps='24')
plt.show()
