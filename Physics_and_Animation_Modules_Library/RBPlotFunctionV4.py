...
def update_line_multiple_segments(xframe,*arg):
    arglen = len(arg)
...
def AnimationFrameSetUp(RBO,ax,plt,istep):
    ...
    ax.set_axis_off()
    ax.view_init(elev=0, azim=RBO.view_azim_angle)
...
def plot_cube_4wires(ax,cube4wires,xframe,zn):
    '''
    Plot the cube from cube vertex data using 4 separate lines each with equal number of segments.
    '''
