# -*- coding: utf-8 -*-
"""
Plot and animation functional/environmental setup for Rigid Body class
...
def getPlotAnimationLines(RBO):
    # no use. '''Construct and return the line animation data sequence in a rigid body object for 3D animation'''
...
def update_line_multiple_segments(x,*arg):
    ...
    arglen = len(arg)
    RBO = arg[0]
    cubeaf = RBO.construct_cube_4wires_method(x)
...
def AnimationFrameSetUp(RBO,ax,plt,istep):
    '''Plot a static istep-th keyframe as the first frame for animation setup. This is needed for plotting the following 3D animation.'''
    ax.set_axis_off()
...
#        RBO.baxes_C= [ax.plot(*[ [0+cordvec[0,j,12]/2,(cordvec[0,j,i]+cordvec[0,j,9])/2] for j in range(3) ]) for i in range(10,13)]
    ...
    if RBO.DrawOption['A_cube'] == True:
        RBO.cubelines = [ ax.plot(*[ [cordvec[istep,j,2]*6.0/10.0,
                                      cordvec[istep,j,polynum[i]]] for j in range(3) ]
                                      ) for i in range(4) ] + [ 
                          ax.plot(*[ [cordvec[istep,j,polynum[i]],
                                      cordvec[istep,j,polynum[i+1]]] for j in range(3) ]
                                      ) for i in range(4) ] + [
                          ax.plot(*[ [cordvec[istep,j,2]*4.0/10.0,
                                      cordvec[istep,j,polynum[i]]] for j in range(3) ]
                                      ) for i in range(4) ]
        plt.setp(RBO.cubelines,lw=2)
        RBO.CubeEdgeT1 = np.hstack([RBO.cordvec[:,:,3],RBO.cordvec[:,:,4]])
        RBO.CubeEdgeT2 = np.hstack([RBO.cordvec[:,:,4],RBO.cordvec[:,:,5]])
        RBO.CubeEdgeT3 = np.hstack([RBO.cordvec[:,:,5],RBO.cordvec[:,:,6]])
        RBO.CubeEdgeT4 = np.hstack([RBO.cordvec[:,:,6],RBO.cordvec[:,:,3]])
        RBO.CubeEdge3 = np.hstack([RBO.cordvec[:,:,3],RBO.cordvec[:,:,4]])
        RBO.CubeEdge4 = np.hstack([RBO.cordvec[:,:,4],RBO.cordvec[:,:,5]])
        RBO.CubeEdge5 = np.hstack([RBO.cordvec[:,:,5],RBO.cordvec[:,:,6]])
        RBO.CubeEdge6 = np.hstack([RBO.cordvec[:,:,6],RBO.cordvec[:,:,3]])
        RBO.linesarg = RBO.linesarg+ (RBO.cubelines[0],RBO.CubeEdge3,
                                        RBO.cubelines[1],RBO.CubeEdge4,
                                        RBO.cubelines[2],RBO.CubeEdge5,
                                        RBO.cubelines[3],RBO.CubeEdge6)
...
#        print fanArrow3d
    ...
    if RBO.DrawOption['B_Angular Velocity Vector (normalized to t0 value)'] == True:
        RBO.w_lab_norm_hasbun = RBO.w_lab_hasbun/RBO.w[0,2]
        lineW=ax.plot([0,RBO.w_lab_norm_hasbun[istep,0]],
                          [0,RBO.w_lab_norm_hasbun[istep,1]],
                          [0,RBO.w_lab_norm_hasbun[istep,2]],'m-',marker='o')
        w_lab_hasbun_normData = np.hstack([np.zeros((RBO.N+1,3)),RBO.w_lab_norm_hasbun])
        RBO.linesarg = RBO.linesarg + (lineW,w_lab_hasbun_normData)


def rotmat_from_A_2_B(A,B):
    '''Return the active r.h. rotation matrix that rotates the world from A axis to B axis.'''
...
def plot_cube_4wires(ax,cube4wires,xframe,zn):
    '''
    No use. Plot the cube from cube vertex data using 4 separate lines each with equal number of segments.
    '''
    cube_111node_aligning_to_z = 0.18*np.dot(rotmat_from_A_2_B(cube4wires[0,:],zn),
                                         np.transpose(cube4wires))#np.transpose(cube4wires)
    for i in range(16):
            cube_111node_aligning_to_z[:,i] = cube_111node_aligning_to_z[:,i] + zn*0.18*np.sqrt(12)/2
