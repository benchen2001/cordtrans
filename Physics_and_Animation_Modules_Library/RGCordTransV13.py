        ...
        if C_UseJCycle == 1:
            ...
            [self.Lx,
             ...
             self.J_euler_angles_hasbun] = top.HasbunMatlabTop(self.Iy,
                                                             ...
                                                             self.theta0d,
                                                             self.psi0d)
            self.J_w_body_hasbun = np.zeros([self.N * 4 + 1,3])
            self.J_w_body_hasbun = self.eulerW2bodyW(self.J_euler_angles_hasbun)
            print np.shape(self.J_w_body_hasbun)
        
        for i in range(1,self.N+1):
            if C_UseJCycle == 1:
                C_rotation_vect_i = CalculateRhoInJCycle(self,self.J_w_body_hasbun[i,:],
                                                         self.J_w_body_hasbun[i+1,:],
                                                         self.J_w_body_hasbun[i+2,:],J_dt)
            else:
